...
```powershell
cargo build --release
iscc /DMyAppVersion=1.2.0 installer\claudometer.iss
```

The installer is written to `dist/Claudometer-Setup-1.2.0.exe`. It installs per-user under `%LOCALAPPDATA%\Programs\Claudometer`, creates a Start Menu shortcut, and starts Claudometer automatically at sign-in through `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`.
